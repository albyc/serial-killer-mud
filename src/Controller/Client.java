...
/**
 * The client side of Serial Killer MUD. This class displays the chat log and
 * sends...
 * 
...
public class Client extends JFrame {
	...
	/**
	 * Sets up the connection between the server and the client. Sets up the
	 * GUI.
	 */
	 ...
	 * 
	 * @param messages
	 *            The current log of messages to display
	 */
	...
	/**
	 * welcomMessage prints out a welcoming message as the player's client
	 * begins
	 * 
	 * @param username
	 *            - player's username
	...
	/**
	 * lists all of the commands a player can use during game play. calls the
	 * update function on the command side of the gui.
	 */
	...
	/**
	 * lists all of the players currently online. updates the command side of
	 * the gui.
	 * 
	 * @param players
	 ...
	 * 
	 * @param argument
	 *            - the string name of the item to be dropped
	 */
	public void dropItem(String argument) {
		...
		if (item != null) {
			...
			dropped = "You no longer have <" + item.getName()
					+ "> in your inventory." + "\n";
		} else
	...
	public void listSurroundings() {
		...
		surroundings += "CurrentRoom:" + name + "\nDescription:"
				...
				+ room.getNamesOfPlayersInRoom() + "\nMOBs in Room:"
				+ room.getNamesOfMOBsInRoom() + "\nItems in Room:"
				+ room.getNamesOfItemsInRoom() + "\nAdjacent Rooms:"
	...
	public void movePlayer(String argument) {
		...
		case "south":
			if (player.getLocation().getSouthRoom() != null) {
				...
				String movedTo = "You have moved south into "
						+ player.getLocation().getRoomName() + "\n";
				commandMessages.add(movedTo);
				mainView.updateCommands(commandMessages);
			} else {
		...
		case "east":
			if (player.getLocation().getEastRoom() != null) {
				...
				String movedTo = "You have moved east into "
						+ player.getLocation().getRoomName() + "\n";
				commandMessages.add(movedTo);
				mainView.updateCommands(commandMessages);
			} else {
		...
		case "west":
			if (player.getLocation().getWestRoom() != null) {
				...
				String movedTo = "You have moved west into "
						+ player.getLocation().getRoomName() + "\n";
				commandMessages.add(movedTo);
				mainView.updateCommands(commandMessages);
			} else {
			...
			break;
		default:
			// message is sent to client letting them know that it is not a
			// valid move
			break;
		}
		listSurroundings();
	...
	/**
	 * Associates this client with a player account. Called by LoginView.
	 * 
	 * @param player
	...
	public void finishSettingUpPlayer() {
		try {
			...
			/*
			 * players.add(player); roomCollection.setRoomsPlayerList(players,
			 * 2);
			 */
			...
			this.addWindowListener(new WindowAdapter() {
				public void windowClosing(WindowEvent arg0) {
					try {
						out.writeObject(new DisconnectCommand(player
								.getUsername()));
						out.close();
						in.close();
					} catch (IOException e) {
	...
	public void updateOne(List<String> messages, String receiver, String sender) {
		if (player.getUsername().equalsIgnoreCase(receiver)
				|| player.getUsername().equalsIgnoreCase(sender)) {
	...
	public void getItemFromPlayer(String item, String player) {
		String getMessage = "You are trying to get <" + item + "> from <"
				+ player + ">. Please wait for their confirmation.";
	...
	public void addConfirmationCommand(String item, String player,
			String clientName) {
		String confirmation = "Would you like to give " + item + " to "
				+ clientName + "?";
	...
	public void addTellMessage(String clientName, String messageReceiver,
			String message) {
		commandMessages.add(clientName + " to " + messageReceiver + ": "
				+ message);
		mainView.updateCommands(commandMessages);
	}
